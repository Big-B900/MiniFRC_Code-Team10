...
float V,W;
float elevatorPos,intakeRoller;
int velocityL,velocityR,wingRelease,spinupCommand,spindownCommand,autoCommand;
...
void loop() {
  while(bluetooth.available()>0){
    if ((bluetooth.read()) == 'z'){
      ...
      spinupCommand = bluetooth.parseInt();
      spindownCommand = bluetooth.parseInt();

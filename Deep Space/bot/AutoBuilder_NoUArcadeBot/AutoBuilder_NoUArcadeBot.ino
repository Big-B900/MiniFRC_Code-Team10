...
int in1 = 18, in2 = 19, climbCommand, auton, cargoAuto;
  ...
  while(j<=50){//repeat until the end of the array (limited at 50 chars for now
    ...
    if(value = 999){///if there is a command 
      ...
      switch(commandType){
        ...
        j+=5;
        break;
        case 2://sensorDrive(float throttle, float rotation, int sensor, int target)
        modA = autoBuilderTasks[j+2];//throttle
        modB = autoBuilderTasks[j+3];//rotation
        modC = (int)autoBuilderTasks[j+4];//sensor
        modD = autoBuilderTasks[j+5];//target
        switch(modC){
          case 0://ultrasonic

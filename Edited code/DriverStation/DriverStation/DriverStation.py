...
class Main:
	def __init__(self):
		...
		#I'm putting my joystick stuff here. I hope it works!
		...
		self.thingsToSend = ["self.speed", "self.turn", "self.intake", "self.extend", "self.arm"]
	...
	def start_loop(self):
		...
		while True:
			...
			for event in pygame.event.get():
				...
				#or event.type == pygame.JOYBUTTONDOWN or event.type == pygame.JOYAXISMOTION or event.type == pygame.JOYHATMOTION
				
				
				"""I removed the elifs from detecting the inputs. Im not sure if that is a good idea or not. 
				
				my reasoning was that I wanted to be able to do multiple actions at once"""
				"""if event.type == pygame.JOYBUTTONDOWN :
					if joystick.get_button(6) == 1 :
						self.intake = -1 #out take
						self.input_dirty = true
					if joystick.get_button(7) == 1 :
						self.intake = 1 #intake
						self.input_dirty = true
				if event.type == pygame.JOYHATMOTION :
					if joystick.get_hat(0) == (0,1) or joystick.get_hat(0) == (1,1) or joystick.get_hat(0) == (-1,1) : # not sure how hats work, make sure it works. 
						self.extend = 1 #extend arm
						self.input_dirty = true
					if joystick.get_hat(0) == (0,-1) or joystick.get_hat(0) == (1,-1) or joystick.get_hat(0) == (-1,-1) : # not sure how hats work, make sure it works. 
						self.extend = -1 #retract arm
						self.input_dirty = true
				# need a better solution for this, so that i can only monitor the ones im looking for and only set true if they have moved enough to be noticable
				if event.type == pygame.JOYAXISMOTION :
					...
					if joystick.get_axis(3) == 1 :
						self.turn = -1 #clockwise
						self.input_dirty = true
					if joystick.get_axis(3) == -1 :
						self.turn = 1 #counterclockwise
						self.input_dirty = true
					if joystick.get_axis(4) == -1:
						self.speed = 1 #forward
						self.input_dirty = true
					if joystick.get_axis(4) == 1 :
						self.speed = 1 #backward
						self.input_dirty = true"""
						
				
				if event.type == pygame.KEYDOWN : # Check if a key is pressed or button or joystick or hat
					if event.key == pygame.K_ESCAPE:
						pygame.quit()
						sys.exit()
						#change these to right stick
				...
				"""if event.type == pygame.JOYBUTTONUP:
					...
					if joystick.get_button(7) == 0 :
						...
						self.input_dirty = true
				if event.type == pygame.JOYHATMOTION :
					if joystick.get_hat(0) == (0,0) or joystick.get_hat(0) == (1,0) or joystick.get_hat(0) == (-1,0) : # not sure how hats work, make sure it works. 
						self.extend = 0
						self.input_dirty = true	
						#stop moving when axis is back around 0
				if event.type == pygame.JOYAXISMOTION :
					if joystick.get_axis(1) < .5 and joystick.get_axis(1) > -.5 : # make sure the cutoff is at a good point
						self.arm = 0 
						self.input_dirty = true
					if joystick.get_axis(3) < .5 and joystick.get_axis(3) > -.5 :
						self.turn = 0
						self.input_dirty = true
					if joystick.get_axis(4) > -.5 and joystick.get_axis(4) < .5 :
						self.speed = 0
						self.input_dirty = true"""
